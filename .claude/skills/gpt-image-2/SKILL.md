...
- 生成图片：`POST /chat/completions`（通过 Chat API 出图，从响应中提取图片 URL 再下载落盘）
- 编辑图片：`POST /images/edits`

---

## ⛔ 强制约束：JS 脚本报错禁止修码 / 禁止改配置（最高优先级）

**本 Skill 是图像生成顾问，不是运维排障工具。** `generate-chat.js` / `edit.js` 执行后如返回任何错误（无论退出码、无论报错内容），你必须严格遵守以下铁律：
...
```bash
node .claude/skills/gpt-image-2/scripts/check-mode.js
# 想拿结构化结果给上层程序用：
node .claude/skills/gpt-image-2/scripts/check-mode.js --json
```

Skill 启动时会自动执行 check-mode.js 检测运行模式。
...
|---|---|---|---|---|
| `ENABLE_GARDEN_IMAGEGEN=1` + 有 KEY | **A** | ✅ `generate-chat.js` / `edit.js` | ✅ 自动 | ✅ 自动 |
...
- `scripts/generate-chat.js`：文本生图，通过 Chat API 出图（仅 Mode A 使用）
...
配置从 `<cwd>/.claude/auth.json` 的 `gateway` 字段强制读取，读不到即报错。
...
输出会告诉你当前是 Mode A / B / C，决定后续是否调用 `generate-chat.js` / `edit.js`。下面 1~4 仅在 **Mode A** 下使用。

### 1. 文本生图（Mode A）

```bash
node .claude/skills/gpt-image-2/scripts/generate-chat.js \
...
- 生成脚本 `generate-chat.js` 走 Chat Completions API（`POST /chat/completions`），body 为 `{model, messages: [{role: "user", content: prompt}]}`
- 响应从 `choices[0].message.content` 中提取图片 URL（Markdown `![](url)` 或裸 URL），再下载落盘
- 编辑脚本 `edit.js` 使用 multipart form data（`POST /images/edits`）
- 所有出图参数（尺寸 / 质量 / 比例等）统一写在 prompt 文本中，不通过 API 参数传递
